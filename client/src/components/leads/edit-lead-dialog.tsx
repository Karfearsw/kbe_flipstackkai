...
export function EditLeadDialog({ open, onOpenChange, lead }: EditLeadDialogProps) {
  ...
  // Populate form with lead data when lead changes
  useEffect(() => {
    if (lead) {
      setFormData({
        ...
        motivationLevel: lead.motivationLevel || "unknown",
        source: lead.source || "other",
        status: lead.status || "new",
        estimatedValue: lead.estimatedValue ? String(lead.estimatedValue) : "",
        arv: lead.arv ? String(lead.arv) : "",
        repairCost: lead.repairCost ? String(lead.repairCost) : "",
  ...
  const updateLeadMutation = useMutation({
    mutationFn: async (data: LeadFormData) => {
      if (!lead) throw new Error("No lead selected for editing");
      ...
      console.log('Updating lead:', processedData);
      const res = await apiRequest("PUT", `/api/leads/${lead.id}`, processedData);
      return res.json();
    },
    onSuccess: (data) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold font-heading text-neutral-900">Edit Lead</DialogTitle>
          <DialogDescription>
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea 
                    placeholder="Additional notes about the lead or property"
